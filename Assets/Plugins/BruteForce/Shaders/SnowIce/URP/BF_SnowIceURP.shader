		...
		[Toggle(USE_LOW)] _USELOW("Use Low End", Float) = 0
			...
			[HideInInspector] _NormalScale7("Normal Scale 7 ", Float) = 1
				...
				[HideInInspector] _Specular7("Specular 7 (A)", Color) = (1,1,1,1)
					...
					// Shadow caster URP variables
					[HideInInspector] _BaseMap("Base Texture", 2D) = "white" {}
					[HideInInspector] _BaseColor("Example Colour", Color) = (0, 0.66, 0.73, 1)
...
#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/SurfaceInput.hlsl"
				...
				struct VertexData //appdata
				{
					...
					InterpolatorsVertex vert(VertexData v) {
...
#ifdef IS_T
						_HeightScale = _HeightScale * 0.333; // correct the heightScale to what it was before update 1.3
#endif
						...
						i.shadowCoord = float4(v.uv.xy * unity_LightmapST.xy + unity_LightmapST.zw,0,0);
#else
...
half gamma = 0.454545;
					...
					float shadowmap = 0;
#ifdef LIGHTMAP_ON
					float3 sampledLightMap = SampleLightmap(i.shadowCoord.xy, normalDirection);
...
lightColor = half4(1,1,1,1);
					...
					float dist = clamp(lerp(0, 1, (distance(_WorldSpaceCameraPos, i.worldPos) - _ProceduralDistance) / max(_ProceduralStrength,0.05)), 0, 1);
								...
								float2 iceUV = float2(i.worldPos.x + _IceTex_ST.z, i.worldPos.z + _IceTex_ST.w) * _HeightScale * 0.1 * _OverallScale;
							#else
								float2 snowUV = float2(i.uv.x * _MainTex_ST.x + _MainTex_ST.z, i.uv.y * _MainTex_ST.y + _MainTex_ST.w);
								float2 iceUV = float2(i.uv.x * _IceTex_ST.x + _IceTex_ST.z, i.uv.y * _IceTex_ST.y + _IceTex_ST.w);
							#endif
								...
								float3 SnowNormal0 = UnpackNormalScale(_Normal0.Sample(my_linear_repeat_sampler, i.uv * (_TerrainScale.xz / _Splat0_ST.xy)), _NormalScale0);
								SnowNormal0 = lerp(SnowNormal0, SnowNormal0, 1 - _Metallic0);
								float3 SnowNormal1 = UnpackNormalScale(_Normal1.Sample(my_linear_repeat_sampler, i.uv * (_TerrainScale.xz / _Splat1_ST.xy)), _NormalScale1);
								SnowNormal1 = lerp(SnowNormal1, SnowNormal1, 1 - _Metallic1);
								float3 SnowNormal2 = UnpackNormalScale(_Normal2.Sample(my_linear_repeat_sampler, i.uv * (_TerrainScale.xz / _Splat2_ST.xy)), _NormalScale2);
								SnowNormal2 = lerp(SnowNormal2, SnowNormal2, 1 - _Metallic2);
								float3 SnowNormal3 = UnpackNormalScale(_Normal3.Sample(my_linear_repeat_sampler, i.uv * (_TerrainScale.xz / _Splat3_ST.xy)), _NormalScale3);
								SnowNormal3 = lerp(SnowNormal3, SnowNormal3, 1 - _Metallic3);

#ifdef USE_COMPLEX_T
								float3 SnowNormal4 = UnpackNormalScale(_Normal4.Sample(my_linear_repeat_sampler, i.uv * (float2(_Splat4_STn.x, _Splat4_STn.y))), _NormalScale4);
								SnowNormal4 = lerp(SnowNormal4, SnowNormal4, 1 - _Metallic4);
								float3 SnowNormal5 = UnpackNormalScale(_Normal5.Sample(my_linear_repeat_sampler, i.uv * (float2(_Splat5_STn.x, _Splat5_STn.y))), _NormalScale5);
								SnowNormal5 = lerp(SnowNormal5, SnowNormal5, 1 - _Metallic5);
								float3 SnowNormal6 = UnpackNormalScale(_Normal6.Sample(my_linear_repeat_sampler, i.uv * (float2(_Splat6_STn.x, _Splat6_STn.y))), _NormalScale6);
								SnowNormal6 = lerp(SnowNormal6, SnowNormal6, 1 - _Metallic6);
								float3 SnowNormal7 = UnpackNormalScale(_Normal7.Sample(my_linear_repeat_sampler, i.uv * (float2(_Splat7_STn.x, _Splat7_STn.y))), _NormalScale7);
								...
								// ICE NORMAL //
								half3 normalTex = lerp(UnpackNormalScale(_Normal0.Sample(my_linear_repeat_sampler, i.uv * _IceScale), _NormalScale), SnowNormal0, splat_control.r * (_Metallic0));
								...
								float3 normal = UnpackNormalScale(_SnowBumpMap.Sample(my_linear_repeat_sampler, (snowUV)*_SnowNormalScale * _SnowScale), _NormalMultiplier * 2).rgb * iceValue - i.normal;
								half3 normalTex = UnpackNormalScale(_NormalTex.Sample(my_linear_repeat_sampler, iceUV * _IceScale), _NormalScale);
					#endif
								...
								//float3 normalDirectionWithNormal = normalize(i.normalDir) + normalize(i.normalDir) * normalize(abs(SampleNormal(i.uv *_SnowNormalScale * _SnowScale,TEXTURE2D_ARGS(_SnowBumpMap, my_linear_repeat_sampler )))) * _NormalMultiplier;
								//float3 normalDirectionWithNormal = normalize(i.normalDir) + normalize(i.normalDir) * normalize((UnpackNormalScale(_SnowBumpMap.Sample(my_linear_repeat_sampler, i.uv * _SnowNormalScale * _SnowScale), _NormalMultiplier).rgb)) * _NormalMultiplier;

#ifdef IS_T
								float3 normalDirectionWithNormal = normalize(i.normalDir) + normalize(i.normalDir) * normal;
#else
								float3 normalDirectionWithNormal = normalize(i.normalDir) + normalize(i.normalDir) * normalize((UnpackNormalScale(_SnowBumpMap.Sample(my_linear_repeat_sampler, snowUV * _SnowNormalScale * _SnowScale), _NormalMultiplier).rgb)) * _NormalMultiplier;
#endif
					...
					float3 albedo = c.rgb;
									...
									float customRatio = 0.5;
										...
										float j1parallax = _Mask0.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat0_ST.xy) + j1UV).r * _ParallaxStrength.y * _Metallic0 * splat_control.r;
										j1parallax += _Mask1.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat1_ST.xy) + j1UV).r * _ParallaxStrength.y * _Metallic1 * splat_control.g;
										j1parallax += _Mask2.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat2_ST.xy) + j1UV).r * _ParallaxStrength.y * _Metallic2 * splat_control.b;
										j1parallax += _Mask3.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat3_ST.xy) + j1UV).r * _ParallaxStrength.y * _Metallic3 * splat_control.a;
					#ifdef USE_COMPLEX_T
										j1parallax += _Mask4.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (float2(_Splat4_STn.x, _Splat4_STn.y)) + j1UV).r * _ParallaxStrength.y * _Metallic4 * splat_control1.r;
										j1parallax += _Mask5.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (float2(_Splat5_STn.x, _Splat5_STn.y)) + j1UV).r * _ParallaxStrength.y * _Metallic5 * splat_control1.g;
										j1parallax += _Mask6.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (float2(_Splat6_STn.x, _Splat6_STn.y)) + j1UV).r * _ParallaxStrength.y * _Metallic6 * splat_control1.b;
										j1parallax += _Mask7.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (float2(_Splat7_STn.x, _Splat7_STn.y)) + j1UV).r * _ParallaxStrength.y * _Metallic7 * splat_control1.a;
					#endif
							...
							for (int j = 1; j < 4; j++)
							{
								...
								else if (j == 2)
								{
									...
									float j2parallax = _Mask0.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat0_ST.xy) + j2UV).b * _ParallaxStrength.z * _Metallic0 * splat_control.r;
									j2parallax += _Mask1.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat1_ST.xy) + j2UV).b * _ParallaxStrength.z * _Metallic1 * splat_control.g;
									j2parallax += _Mask2.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat2_ST.xy) + j2UV).b * _ParallaxStrength.z * _Metallic2 * splat_control.b;
									j2parallax += _Mask3.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat3_ST.xy) + j2UV).b * _ParallaxStrength.z * _Metallic3 * splat_control.a;
				#ifdef USE_COMPLEX_T
									j2parallax += _Mask4.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (float2(_Splat4_STn.x, _Splat4_STn.y)) + j2UV).b * _ParallaxStrength.z * _Metallic4 * splat_control1.r;
									j2parallax += _Mask5.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (float2(_Splat5_STn.x, _Splat5_STn.y)) + j2UV).b * _ParallaxStrength.z * _Metallic5 * splat_control1.g;
									j2parallax += _Mask6.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (float2(_Splat6_STn.x, _Splat6_STn.y)) + j2UV).b * _ParallaxStrength.z * _Metallic6 * splat_control1.b;
									j2parallax += _Mask7.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (float2(_Splat7_STn.x, _Splat7_STn.y)) + j2UV).b * _ParallaxStrength.z * _Metallic7 * splat_control1.a;
				#endif
								...
								else if (j == 3)
								{
									...
									float j3parallax = _Mask0.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat0_ST.xy) + j3UV).g * _ParallaxStrength.w * _Metallic0 * splat_control.r;
									j3parallax += _Mask1.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat1_ST.xy) + j3UV).g * _ParallaxStrength.w * _Metallic1 * splat_control.g;
									j3parallax += _Mask2.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat2_ST.xy) + j3UV).g * _ParallaxStrength.w * _Metallic2 * splat_control.b;
									j3parallax += _Mask3.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat3_ST.xy) + j3UV).g * _ParallaxStrength.w * _Metallic3 * splat_control.a;
				#ifdef USE_COMPLEX_T
									j3parallax += _Mask4.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (float2(_Splat4_STn.x, _Splat4_STn.y)) + j3UV).g * _ParallaxStrength.w * _Metallic4 * splat_control1.r;
									j3parallax += _Mask5.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (float2(_Splat5_STn.x, _Splat5_STn.y)) + j3UV).g * _ParallaxStrength.w * _Metallic5 * splat_control1.g;
									j3parallax += _Mask6.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (float2(_Splat6_STn.x, _Splat6_STn.y)) + j3UV).g * _ParallaxStrength.w * _Metallic6 * splat_control1.b;
									j3parallax += _Mask7.Sample(my_linear_repeat_sampler, float4(iceUV, 0, 0) * _IceScale * (float2(_Splat7_STn.x, _Splat7_STn.y)) + j3UV).g * _ParallaxStrength.w * _Metallic7 * splat_control1.a;
				#endif
							...
							else
							{
								...
								float3 normalSample = lerp(normalTex, UnpackNormalScale(_SnowBumpMap.Sample(my_linear_repeat_sampler, snowUV * _SnowScale), _NormalMultiplier * 3).rgb , iceValue);
					...
					#else
								float4 specGloss = pow(tex2D(_SpecGlossMapNew, snowUV * 2 * _SnowScale), _SpecForce);
...
#else
					#ifdef IS_T
								half rougnessTemp = _Mask0.Sample(my_linear_repeat_sampler, float4(i.uv.xy, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat0_ST.xy)).r * 2 * _RoughnessStrength;

								rougnessTemp = lerp(rougnessTemp, _Mask1.Sample(my_linear_repeat_sampler, float4(i.uv.xy, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat1_ST.xy)).r * 2 * _RoughnessStrength, _Metallic1 * splat_control.g);
								rougnessTemp = lerp(rougnessTemp, _Mask2.Sample(my_linear_repeat_sampler, float4(i.uv.xy, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat2_ST.xy)).r * 2 * _RoughnessStrength, _Metallic2 * splat_control.b);
								rougnessTemp = lerp(rougnessTemp, _Mask3.Sample(my_linear_repeat_sampler, float4(i.uv.xy, 0, 0) * _IceScale * (_TerrainScale.xz / _Splat3_ST.xy)).r * 2 * _RoughnessStrength, _Metallic3 * splat_control.a);

					#ifdef USE_COMPLEX_T
								rougnessTemp = lerp(rougnessTemp, _Mask4.Sample(my_linear_repeat_sampler, float4(i.uv.xy, 0, 0) * _IceScale * (float2(_Splat4_STn.x, _Splat4_STn.y))).r * 2 * _RoughnessStrength, _Metallic4 * splat_control1.r);
								rougnessTemp = lerp(rougnessTemp, _Mask5.Sample(my_linear_repeat_sampler, float4(i.uv.xy, 0, 0) * _IceScale * (float2(_Splat5_STn.x, _Splat5_STn.y))).r * 2 * _RoughnessStrength, _Metallic5 * splat_control1.g);
								rougnessTemp = lerp(rougnessTemp, _Mask6.Sample(my_linear_repeat_sampler, float4(i.uv.xy, 0, 0) * _IceScale * (float2(_Splat6_STn.x, _Splat6_STn.y))).r * 2 * _RoughnessStrength, _Metallic6 * splat_control1.b);
								rougnessTemp = lerp(rougnessTemp, _Mask7.Sample(my_linear_repeat_sampler, float4(i.uv.xy, 0, 0) * _IceScale * (float2(_Splat7_STn.x, _Splat7_STn.y))).r * 2 * _RoughnessStrength, _Metallic7 * splat_control1.a);
					#endif
					...
					rougnessTex = lerp(0, rougnessTex, saturate(max(0.3, lightIntensity * 4) * lightColor.a));
								...
								smallSpec = saturate(smallSpec * saturate(UnpackNormalScale(_NormalTex.Sample(my_linear_repeat_sampler, iceUV * _IceScale), 0.01 * _NormalScale).rgb * 20 + 0.8));
								...
								//return float4(diff,diff,diff, transparency);
								//return float4(customOUT, transparency);

								float littleSun = smallSpec;
								//return float4(littleSun, littleSun, littleSun, transparency);
				...
				struct VertexData //appdata
				{
					...
					InterpolatorsVertex vert(VertexData v) {
						...
						// this is important for correct vertex position when using shadow casting with custom vertex pass 
#if UNITY_REVERSED_Z
						i.pos.z = min(i.pos.z, i.pos.w * UNITY_NEAR_CLIP_VALUE);
#else
						i.pos.z = max(i.pos.z, i.pos.w * UNITY_NEAR_CLIP_VALUE);
#endif
